...
c
      ...
      real tval,dtval,wrk1

      dtval = 0.
      if (istep.eq.0) then
        open(50,file='dt.dat',status='old',err=100)
        read(50,*,err=100) dtval
        close(50)
        dtval = glmax(dtval,1)
        nsteps = 0.1/dtval
        iostep = 0.01/dtval
...
c    Global error calculation
      ...
      if (nid.eq.0) then
          write(6,11) istep,time,dt,udx,umx,uex,u0,'  X err', session
          write(6,11) istep,time,dt,vdx,vmx,vex,v0,'  Y err', session
      end if
      call nekgsync
      if (nid_global.eq.0) then	  
          write(6,11) istep,time,dt,udx_gl,umx_gl,uex_gl,u0,'  X err',
     & 'global'
          write(6,11) istep,time,dt,vdx_gl,vmx_gl,vex_gl,v0,'  Y err',
     & 'global'
        if (istep.eq.nsteps) then
          write(6,11) istep,time,dt,udx_gl,umx_gl,uex_gl,u0,' k10err',
     & 'global'
          write(6,11) istep,time,dt,vdx_gl,vmx_gl,vex_gl,v0,' k10err',
     & 'global'
        endif
   11     format(i5,1p6e14.6,a7,2x,a7)
      endif
...
c     NOTE ::: This subroutine MAY NOT be called by every process
      ...
      if (igeom.le.2) then
         ...
         if (nfld_neknek.gt.3) temp = ubc(ix,iy,iz,ie,4)
      else
...
c     nfld_neknek = 3 - u,v,w velocitites  
